...
def manage_accounts():
    ...
    if st.button("Tambah Pengguna"):
        if not email or not username or not password:
            st.error("Harap isi email, username, atau password terlebih dahulu")
        elif "@gmail.com" not in email:
            st.error("Email harus mengandung '@gmail.com'.")
        else:
            # Mengecek apakah username sudah ada
            existing_users = read_users()
            usernames = [user[2] for user in existing_users]
            
            if username in usernames:
                st.error("Username sudah ada, silakan masukkan username lain")
            else:
